...
function render(state) {
    if (state == "free") {
        return "<img src='/img/maptile0.png' />"
    } else if (state == "bush") {
        return "<img src='/img/maptile1.png' />"
    } else if ( state == true){
        return "";
    } else {
        state.element.style.backgroundImage = `url('/img/overworld/${state.face}/${state.type}.png')`;
        state.element.style.left = `${state.coords[0] * 32}px`
        state.element.style.top = `${state.coords[1] * 32}px`
        divCont.appendChild(state.element)
        return "<img src='/img/maptile0.png' />"
    }
}

cfg.units.push(new WildPokemon(150, [16,14]))
...
function defaultTurn() {
    ...
    if (cfg.time % 25 == 0) {
        ...
        //cfg.units.push(new WildPokemon(6, [5,6]))
        cfg.units.push(new WildPokemon(Math.floor(Math.random()*350), [Math.floor(Math.random()*cfg.mapSize[0]),Math.floor(Math.random()*cfg.mapSize[1])]))
